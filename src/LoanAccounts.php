				...
				<table>
					...
					foreach ($acts["loans"] as $chart => $act)
					{
						...
						foreach($act["xtns"] as $j)
						{
							...
							$cr = "";
							$dr = "";
							if ($j->journal_gross > 0 )
								$cr = number_format($j->journal_gross,2);
							if ($j->journal_gross < 0 )
								$dr = number_format(-$j->journal_gross,2);
							//$desc = $j->other_chart;
							$desc = $acts["chart_descriptions"][$j->other_chart];
							$type = $acts["chart_types"][$j->other_chart];
							if ($type == 'cash' && $j->journal_gross < 0)
								$desc = "Draw Down";
							if ($type == 'cash' && $j->journal_gross > 0)
                                $desc = "Pay off";

							$balance -= $j->journal_gross;
							$strBalance = LedgerAmount::format1($balance);
