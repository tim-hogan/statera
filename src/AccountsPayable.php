...
if ($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if (strlen($errmsg) == 0)
    {
        ...
        $xtn = $DB->payAccountsPayable($xtn,$amount);
        if ($xtn)
        {
        ...
        <div id="main">
            ...
            <div id="table">
                <table>
                    ...
                    {
                        echo "<tr><th>DATE</th><th>DESCRIPTION</th><th>VENDOR</th><th class='r'>AMMOUNT OWING</th></tr>";
                        foreach($accts as $act)
                        {
                            ...
                            echo "<tr><td>{$strdt}</td><td class='tddesc'>{$desc}</td><td class='tddesc'>{$vendname}</td><td class='r'>{$strAmount}</td><td><input class='r' type='txt' _xtn='{$act['journal_xtn']}' name='pay[]' value='$strAmount' size='10' /></td><td><button onclick='pay(this)'>PAY</button></td></tr>";
                        }
                    }
                    ?>
                </table>
