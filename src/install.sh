...
if $INSTALLFILES ; then
	...
	echo -e "Saving off attachments"
	
	cp -r $WEBDIR/attachements /tmp/$WEBDIR/attachements
	...
	cp -r /tmp/$WEBDIR/attachements $WEBDIR/attachements
...
if $INSTALLDB ; then
	...
	if $INSTALLDB ; then
		...
		if [ "$DUMMY" == "Y" ] ; then
			...
			vault add -s $INSTALLNAME -k QUOTE_KEY -v $QUOTE_KEY
